...
export class ScheduleAppointmentComponent {
  ...
  submitScheduleForm(): void {
    ...
    if (isNaN(date.getTime()) || date < new Date()) {
      this.errorMessage = 'Appointment date must be valid and at least from the current day.';
      return;
    }
    ...
    if (date.getMinutes() < 15) {
      this.errorMessage = 'Appointment must be scheduled at least 15 minutes from now.';
      return;
    }
